...
! -------------------------------------------------------------------------------------------------------------------------------------
      ...
      subroutine interfase21(ndimk1x, ndimk1y, ndim1, ndimk2x, ndimk2y, 
     &                    ndim2, n1x, n1y, n2x, n2y, f1m, f2p, f2m, 
     &                    sol, nummod1, nummod2, numte, im1, im2, amat1,
     &                    conex12, conex21, wbiga, wa, rhol22, tau21) 
c
     ...
     &           conex21(ndim1, ndim2), wbiga(ndim2, ndim2), 
     &           rhol22(ndim2, ndim2), tau21(ndim1, ndim2)
c
      cz = dcmplx(0.d0, 0.d0)
c
      iequation = 0
        ...
        do k2eq = 1, numte
           ...
           do 20 neta = 3, 4
              ...
              if (jeq.eq.j0 .and. ieq.eq.i0) then
                 rhol22(iequation, k20) = f2m(ipol20,i0,j0,neta)
              else
                 rhol22(iequation, k20) = cz
              end if
              do k1 = 1, nummod1
                 ipol1 = im1(k1,0)
                 m = im1(k1,1)
                 n = im1(k1,2)
                 rhol22(iequation, k20) = rhol22(iequation, k20) -
     &              dconjg(sol(m,n,k2eq,netas)) * f1m(ipol1,m,n,neta) *
     &              conex21(k1,k20)
              end do
20         continue
        end do
...
c     reflection 2->2
c
      call LEQT1C(wbiga,nummod2,ndim2,rhol22,nummod2,ndim2,0,WA,IER)
c
c     transmission 2->1
c
       do k20 = 1, nummod2
          do k1 = 1, nummod1
             tau21(k1,k20) = conex21(k1,k20)
             do k2 = 1, nummod2
                tau21(k1,k20) = tau21(k1,k20) + 
     &                          conex12(k1,k2)*rhol22(k2,k20)
             end do
          end do
       end do
c
